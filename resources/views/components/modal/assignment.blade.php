<aside id="assignment" class="modal">
    <section class="modal-content center">
        <form class="pr-6" id="assignment-form" action="#" method="post" class="grid">
            @csrf
            @method("post")
            <main class="pl-12 py-12 pr-6">
                ...
                <div class="input-group grid mt-2 mb-6">
                    ...
                    <input class="mb-4 px-5 py-4 overpass assignment-form form-input" id="assignment-url" type="text" placeholder="http://...." name="url">
                    <div id="assignment-video"></div>
                    @if ($errors->has("url"))
                        <span class="color-white error support assignment-form support-box overpass mb-4 support-url">{{ $errors->first("url") }}</span>
                    @else
                        <span class="color-white error support assignment-form support-box overpass mb-4 support-url"></span>
                    @endif
                </div>
                
                <div class="input-group grid mb-2">
                    <h3 class="color-four mb-4 overpass">Describi que queres mejorar:</h3>
                    <textarea placeholder="Si no sabés por donde empezar, podes usar estas preguntas como guia: ¿Hay algo en particular que te gustaria mejorar? ¿Qué sentis que te sale bien, y que mal, cuando jugas? ¿Jugas solo o en equipo? ¿Tenes una rutina de entrenamiento?" class="descripcion mb-4 px-5 py-4 overpass assignment-form form-input" name="description" cols="30" rows="10"></textarea>
                    @if ($errors->has("description"))
                        <span class="color-white error support assignment-form support-box overpass mb-4 support-description">{{ $errors->first("description") }}</span>
                    @else
                        <span class="color-white error support assignment-form support-box overpass mb-4 support-description"></span>
                    @endif
                ...
                <div>
                    <button class="btn btn-background btn-one form-submit assignment-form flex justify-center w-full rounded p-1 md:h-12 md:items-center mt-12 russo" type="submit">
