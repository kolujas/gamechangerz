                ...
                <td style="background-color: #281B2D;">
                    <div style="color: #34495e; margin: 4% 10% 2%; text-align: justify;font-family: sans-serif; border-radius: 0.25rem;">
                        @switch($data->lesson->type->id_type)
                            @case(1)
                            @case(3)
                                <h2 style="text-align: center; color: #ED6744;margin: 20px 0;">Felicitaciones! Se reservó una nueva clase (1on1) por con el coach {{ $data->lesson->users->from->username }}</h2>
                                <p style="margin: 2px;padding-top: 2rem;text-align: center;font-family: sans-serif;font-size: 17px;min-height: 70px;background-color: #f8f8f8;padding: 1rem 1rem;margin-bottom: 2.5rem;">Recordá que deberás encontrarte con él en el canal <i>“Sala de espera para clases”</i> de nuestro canal de Discord a la hora de inicio de la clase. Vas a poder identificar a tu coach con el nick de Discord {{ $data->lessons->users->from->discord }}. Luego de eso, él te redirigirá a uno de nuestros <i>"Coaching Rooms"</i> para asegurarte de que nadie más pueda entrar e interrumpir la clase.</p>
                                ...
                                <p style="margin: 2px;padding-top: 2rem;text-align: center;font-family: sans-serif;font-size: 17px;min-height: 70px;background-color: #f8f8f8;padding: 1rem 1rem;margin-bottom: 2.5rem;">Mucha suerte y preparate para <i>#CambiarElJuego</i></p>                                
                                @break
                            @case(2)
                                <h2 style="text-align: center; color: #ED6744;margin: 20px 0;">Felicitaciones! Reservaste un Seguimiento Online con el coach {{ $data->lesson->users->from->username }}</h2>
